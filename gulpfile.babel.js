// Generated on 2016-08-03 using generator-angular-fullstack 3.7.6
'use strict';
...
const paths = {
    client: {
        ...
        styles: [`${clientPath}/{app,components}/**/*.scss`, `!${clientPath}/{app,components}/core/scss/global.scss`],
    ...
    return gulp.src(paths.client.mainView)
        .pipe(plugins.inject(
            gulp.src(_.union(paths.client.scripts, [`!${clientPath}/**/*.{spec,mock}.js`, `!${clientPath}/app/app.js`]))
                ...
                .pipe(plugins.angularFilesort()).on('error', errorHandler('AngularFilesort')),
            {
...
gulp.task('inject:scss', () => {
    return gulp.src(paths.client.mainStyle)
        .pipe(plugins.inject(
            gulp.src(_.union(paths.client.styles, ['!' + paths.client.mainStyle, '!' + paths.client.globalStyle]), {read: false})
                .pipe(plugins.sort()),
            {
                transform: (filepath) => {
...
gulp.task('production', ['start:server:prod', 'start:client']);
...
gulp.task('build:client', ['styles', 'html', 'constant', 'build:images'], () => {
    ...
    gulp.src(paths.client.mainView)
        .pipe(plugins.useref())
            .pipe(cssFilter)
                ...
                .pipe(plugins.concat('app/app.css'))
    ...
    gulp.src("./client/app/data/**/*")
        .pipe(gulp.dest(`${paths.dist}/${clientPath}`+"/app/data"));

    gulp.src("./client/app/**/*.json")
